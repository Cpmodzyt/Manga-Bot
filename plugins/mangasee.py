...
class McReaderClient(MangaClient):
    ...
    def chapters_from_page(self, page: bytes, manga: MangaCard = None):
        ...
        ch_names = [item.get("title") for item in items]
        texts = []
        for ch_name in ch_names:
            match = re.search(r"chapter-([\d]+(?:\.[\d]+)?)\-([\w-]+)", ch_name)
            texts.append(f"{match.group(1)}-{match.group(2)}")
    ...
    async def updates_from_page(self, page: bytes):
        ...
        for manga_item in manga_items:
            ...
            ul = scrap.find('li', {'class': 'chapter-list-item'})
            for u in ul: link = u.findNext('a').get('href')
            chapter_url = urljoin(self.base_url.geturl(), link)
